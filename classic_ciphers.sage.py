...
_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_10 = Integer(10); _sage_const_26 = Integer(26); _sage_const_25 = Integer(25)
import os.path
import re as regex
import random
import math
...
Vigenere Cipher
"""
alpha_num = [chr(i) for i in range (ord('A'),ord('Z')+_sage_const_1 )]
num = [i for i in range(_sage_const_26 )]
dict = {k: v for k, v in zip(alpha_num, num)}
...
def key_gen_vig(key_length):
    ...
    for x in xrange(_sage_const_0 , key_length):
        key.append(Permutations(alpha_num).random_element()[_sage_const_0 ])
    key = ''.join(key)
...
def enc_vig(key, plain_txt, cipher_txt):
    ...
    for i in xrange(_sage_const_0 , len(plain_txt_list)):
            val = (dict[plain_txt_list[i]] + dict[key_list[i % len(key_list)]]) % _sage_const_26 
...
def dec_vig(key, cipher_txt, plain_txt):
    ...
    for i in xrange(_sage_const_0 , len(cipher_txt_list)):
        val = (dict[cipher_txt_list[i]] - dict[key_list[i % len(key_list)]]) % _sage_const_26 
