...
_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_10 = Integer(10); _sage_const_26 = Integer(26); _sage_const_25 = Integer(25)
import os.path
import re as regex
import random
import math

alphabet = [chr(i) for i in range (ord('A'),ord('Z')+_sage_const_1 )]
num = [i for i in range(_sage_const_26 )]
...
def key_gen_aff():
    a_set = {x for x in xrange(_sage_const_2 ,_sage_const_26 ) if gcd(x,_sage_const_26 ) == _sage_const_1 }
    b_set = {y for y in xrange(_sage_const_0 ,_sage_const_26 )}
    print "affine cipher key: " + str((random.sample(a_set,_sage_const_1 )[_sage_const_0 ],random.sample(b_set,_sage_const_1 )[_sage_const_0 ]))

    return (random.sample(a_set,_sage_const_1 )[_sage_const_0 ],random.sample(b_set,_sage_const_1 )[_sage_const_0 ])
...
def enc_aff(key, plain_txt, cipher_txt):
    alpha = key[_sage_const_0 ]
    beta = key[_sage_const_1 ]
    plain_txt_list = list(plain_txt)
    cipher_txt_list = []
    for i in xrange(_sage_const_0 , len(plain_txt_list)):
        val = (dictionary[plain_txt_list[i]] * alpha + beta) % _sage_const_26 
...
def dec_aff(key, cipher_txt, plain_txt):
    alpha = key[_sage_const_0 ]
    beta = key[_sage_const_1 ]
    alpha_inverse = inverse_mod(alpha,_sage_const_26 )
    cipher_txt_list = list(cipher_txt)
    plain_txt_list = []
    for i in xrange(_sage_const_0 ,len(cipher_txt_list)):
        val = ((dictionary[cipher_txt_list[i]] - beta) * alpha_inverse) % _sage_const_26 
...
def key_gen_vig(key_length):
    ...
    for x in xrange(_sage_const_0 , key_length):
        key.append(Permutations(alphabet).random_element()[_sage_const_0 ])
    key = ''.join(key)
...
def enc_vig(key, plain_txt, cipher_txt):
    ...
    cipher_txt_list = []
    for i in xrange(_sage_const_0 , len(plain_txt_list)):
            val = (dictionary[plain_txt_list[i]] + dictionary[key_list[i % len(key_list)]]) % _sage_const_26 
            cipher_txt_list.append(alphabet[val])
...
def dec_vig(key, cipher_txt, plain_txt):
    ...
    cipher_txt_list = list(cipher_txt)
    for i in xrange(_sage_const_0 , len(cipher_txt_list)):
        val = (dictionary[cipher_txt_list[i]] - dictionary[key_list[i % len(key_list)]]) % _sage_const_26 
        plain_txt_list.append(alphabet[val])
    plain_txt = ''.join(plain_txt_list)
    print "decrypted text: " + plain_txt
